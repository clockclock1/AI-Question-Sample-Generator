...
- 内置“编译并运行”面板：可像编译器一样手动编译检查并试运行代码
- 自动运行代码并测试校验
...
- 配置会自动持久化到根目录 `app_config.json`，下次启动自动加载
- 打包版执行器已改为内置子进程沙箱执行代码，避免误调用应用自身
- 导出前会尽量扩展并补全更多覆盖样例（去重后最多 24 组）
...
3. `正确代码` 区域：
   ...
   - 可在“代码运行器”中输入 `stdin`，点“编译检查”或“编译并运行”查看即时结果。
4. 添加测试用例（可多组）：
   - `Input` 必填建议；
